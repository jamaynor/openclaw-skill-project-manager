...
function extractBody(content) {
  // Normalize CRLF → LF so frontmatter markers are found regardless of line endings.
  // Return the normalized body (consumers write fresh frontmatter + LF-normalized body anyway).
  const c = content.replace(/\r\n/g, '\n');
  if (!c.startsWith(MARKER + '\n')) return c;
  // Standard: closing --- followed by newline
  const endIdx = c.indexOf('\n' + MARKER + '\n', MARKER.length);
  if (endIdx !== -1) {
    return c.slice(endIdx + MARKER.length + 2); // skip \n---\n
  }
...
export { buildFrontmatter, extractBody, setFrontmatter };
