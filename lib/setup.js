...
async function promptYesNo(rl, question, defaultVal = true) {
...
async function runSetup(workspace) {
  ...
  if (existingConfig) {
    ...
    if (!overwrite) { rl.close(); console.log('Aborted.'); return; }
  }
  ...
  // ---- Naming convention ------------------------------------------------
  console.log('\n--- Naming Convention ---');
  console.log('Default: yyyy.mm.dd-{location}-{slug}  (location is omitted for local roots)');
  const useDefault = await promptYesNo(rl, 'Use default naming convention?', true);
  config.namingConvention = useDefault ? 'yyyy.mm.dd-{location}-{slug}' :
    await prompt(rl, 'Custom convention (use {location} and {slug} as placeholders)');
  ...
  while (addMore) {
    console.log('');
    const name        = await prompt(rl, 'Root name (e.g. asd-vault, personal)');
    const vaultPath   = await prompt(rl, 'Full path to vault (e.g. /vaults/asd-vault)');
    const subfolder   = await prompt(rl, 'Projects subfolder inside vault', '1-Projects');
    const location    = await prompt(rl, 'Location code for naming (e.g. asd, lmb, ja)');
    const description = await prompt(rl, 'Description (optional)', '');

    config.roots.push({
      name,
      type:     'vault',
      path:     path.join(vaultPath, subfolder),
  ...
  console.log('\nNext: project-mgmt create --name "My Project" --root <root-name>');
}
