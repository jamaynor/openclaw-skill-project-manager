...
// ---------------------------------------------------------------------------
function writeSystemConfig(workspace, config) {
  const configDir  = process.env.HAL_SYSTEM_CONFIG || '/data/openclaw/config';
  ...
  current.skills['project-manager'] = {
    ...
    'naming-convention': config ? config.namingConvention : 'yyyy.mm.dd-{location}-{slug}',
...
async function runSetup(defaultWorkspace) {
  ...
  try {
    ...
    const sysConfigPath = writeSystemConfig(workspace, config);
