...
function writeSystemConfig(workspace, config, vaultList) {
  ...
  let current = { version: '1.0', 'hal-shared-resources': { 'hal-vault-root': '/vaults', vaults: [] }, skills: {} };
  try {
    current = JSON.parse(fs.readFileSync(configFile, 'utf8'));
  } catch { /* first write — use defaults */ }
  current.skills = current.skills || {};
  current['hal-shared-resources'] = current['hal-shared-resources'] || {};

  // Merge detected vaults into hal-shared-resources.vaults
  if (vaultList && vaultList.length > 0) {
    const existing = current['hal-shared-resources'].vaults || [];
    ...
    current['hal-shared-resources'].vaults = merged;
  }
