...
function run(workspace, args) {
  ...
  if (opts.status && !KNOWN_STATUSES.includes(opts.status)) {
    console.error(`ERROR: Unknown status '${opts.status}'. Valid values: ${KNOWN_STATUSES.join(', ')}`);
    process.exit(1);
  }
  ...
  for (const p of projects) {
    if (KNOWN_STATUSES.includes(p.status)) {
      ...
      console.warn(`WARN: Project '${p.id}' has unrecognised status '${p.status}' — listed under unknown`);
