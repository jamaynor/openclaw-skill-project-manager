...
// Priority: --agent-workspace flag > HAL_AGENT_WORKSPACE env > cwd
// ---------------------------------------------------------------------------
function resolveAgentWorkspace(args) {
  if (args) {
    const agentFlagIdx = args.indexOf('--agent-workspace');
    if (agentFlagIdx !== -1) {
      if (!args[agentFlagIdx + 1] || args[agentFlagIdx + 1].startsWith('--')) {
        throw new Error('ERROR: --agent-workspace requires a path argument');
      }
      return path.resolve(args[agentFlagIdx + 1]);
    }
  ...
  if (process.env.HAL_AGENT_WORKSPACE) return path.resolve(process.env.HAL_AGENT_WORKSPACE);
  return process.cwd();
}

// Flags that take no value — everything else consumes the next token as its value.
const BOOLEAN_FLAGS = new Set(['json']);

// ---------------------------------------------------------------------------
// Shared CLI argument parser
// Skips --workspace and --agent-workspace (already consumed upstream).
...
function parseArgs(args) {
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    if (args[i] === '--workspace' || args[i] === '--agent-workspace') { i++; continue; }
    ...
    if (BOOLEAN_FLAGS.has(key)) {
      ...
      if (!hasValue && i + 1 < args.length) {
        // Next token starts with '--': likely a missing value for this flag
        console.warn(`WARN: --${key} value looks like a flag ('${args[i + 1]}'). Did you forget to provide a value?`);
        log.warn('flag value looks like another flag', { flag: key, value: args[i + 1] });
      }
      val = hasValue ? args[++i] : true;
    }
    if (key in opts) {
...
function loadIndex(workspace) {
  ...
  try {
    ...
    log.debug('index loaded', { path: idxFile, projectCount: idx.projects.length });
    return idx;
  } catch (e) {
...
function saveIndex(workspace, index) {
  ...
  log.debug('index saved', { path: idxFile });
}
